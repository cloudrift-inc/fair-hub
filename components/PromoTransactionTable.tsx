...
interface PromoTransactionTableProps {
  transactions: PromoTransaction[];
}

const formatAmount = (amountInCents: number) => (amountInCents / 100).toFixed(2);

const PromoTransactionTable: React.FC<PromoTransactionTableProps> = ({ transactions }) => {
  ...
  const filterTransactions = (transactions: PromoTransaction[]) => {
    const now = new Date();
        ...
        <table className="w-full text-left min-w-max">
          ...
          <tbody>
            {filterTransactions(transactions).map((transaction, index) => (
              <tr
                key={`${transaction.created_at}-${transaction.amount}`}
...
export default PromoTransactionTable;